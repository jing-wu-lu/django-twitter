...
class HBaseModel:
    ...
    def __init__(self, **kwargs):
        for key, field in self.get_field_hash().items():
            ...
            setattr(self, key, value)  # add into __dict__
    ...
    def serialize_field(cls, field, value):
        ...
        if isinstance(field, IntegerField):
            ...
            # 解决的办法是固定 int 的位数为 16 位（8 的倍数更容易利用空间），不足位补 0
            value = str(value)
            while len(value) < 16:
                value = '0' + value
        if field.reverse:
    ...
    def save(self):
        ...
        # 如果 row_data 为空，即没有任何任何 column key values 需要存储， hbase 会直接不存储这个 row_key
